...
class BasicWrapPolicy:
    ...
    def _invoke_(self, dispid, lcid, wFlags, args):
        ...
        # a custom policy to define _invokeex_, and automatically get _invoke_
        # too.
        return S_OK, - \
            1, self._invokeex_(dispid, lcid, wFlags, args, None, None)
    ...
    # policies to explicitely implement all this functionality just to not
    # implement it!
        ...
        if isinstance(dispid, type("")):
            try:
                ...
                raise COMException(
                    scode=winerror.DISP_E_MEMBERNOTFOUND,
                    desc="Member not found")
        return self._invokeex_(dispid, lcid, wFlags,
    ...
    def _getnextdispid_(self, fdex, dispid):
        ids = sorted(self._name_to_dispid_.values())
...
class DesignatedWrapPolicy(MappedWrapPolicy):
    ...
    def _wrap_(self, ob):
        ...
        if not hasattr(ob, '_public_methods_') and not hasattr(
                ob, "_typelib_guid_"):
            raise error(
    ...
    def _build_typeinfos_(self):
        ...
        # Look for the first name in _com_interfaces_ that exists in the
        # typelib.
    ...
    def _invokeex_(self, dispid, lcid, wFlags, args, kwArgs, serviceProvider):
        ...
        if wFlags & DISPATCH_METHOD:
            try:
                ...
                try:
                    ...
                    # May have a dispid, but that doesnt mean we have the
                    # function!
                    raise COMException(scode=winerror.DISP_E_MEMBERNOTFOUND)
        ...
        if wFlags & DISPATCH_PROPERTYGET:
            ...
            if isinstance(retob, types.MethodType):
                retob = retob(*args)
            return retob

        if wFlags & (DISPATCH_PROPERTYPUT |
                     DISPATCH_PROPERTYPUTREF):  # correct?
            try:
...
class EventHandlerPolicy(DesignatedWrapPolicy):
    ...
    def _transform_args_(self, args, kwArgs, dispid,
