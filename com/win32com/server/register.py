...
def _find_localserver_module():
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                "Can not locate the Python module 'win32com.server.%s'" %
                baseName)
    return pyfile
...
def RegisterServer(clsid,
    ...
    if clsctx & pythoncom.CLSCTX_INPROC_SERVER:
        ...
        if pythoncom.frozen:
            ...
            # manifest issues (the 'loader' DLL has a manifest, but pythoncom
            # does not)
            ...
            if pythoncom.__file__.find("_d") < 0:
                ...
                suffix = "_d"
            loadername = "pythoncomloader%d%d%s.dll" % (
...
def UnregisterInfoClasses(*classes, **flags):
    ...
    for cls in classes:
        ...
        ret = ret + \
            GetUnregisterServerKeys(clsid, progID, verProgID, customKeys)
    return ret
