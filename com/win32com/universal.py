...
def RegisterInterfaces(typelibGUID, lcid, major, minor, interface_names=None):
    ...
    # First see if we have makepy support.  If so, we can probably satisfy the
    # request without loading the typelib.
    try:
    ...
    if mod is None:
        ...
        for name in interface_names:
            ...
            if type_info is None:
                raise ValueError(
                    "The interface '%s' can not be located" %
                    (name,))
            # If we got back a Dispatch interface, convert to the real
            # interface.
...
class Definition:
    ...
    def dispatch(self, ob, index, argPtr,
        ...
        # Like pythoncom, we special case a tuple.
