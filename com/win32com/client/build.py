...
class MapEntry:
    ...
    def __init__(self, desc_or_id, names=None, doc=None,
        ...
        if isinstance(desc_or_id, type(0)):
...
class DispatchItem(OleItem):
    ...
    def _AddFunc_(self, typeinfo, fdesc, bForUser):
        ...
        # We need to translate any Alias', Enums, structs etc in result and
        # args
...
#			sys.stderr.write("%s\n" % (argDesc[0],))
        ...
        # (in vb, PUT=="obj=other_obj", PUTREF="set obj=other_obj
            ...
            if existing is not None:
                if existing.desc[
                        4] == pythoncom.INVOKE_PROPERTYPUT:  # Keep this one
                    map = self.mapFuncs
                    name = "Set" + name
    ...
    def MakeFuncMethod(self, entry, name, bMakeClass=1):
        ...
        if entry.desc is not None and (
                len(entry.desc) < 6 or entry.desc[6] != -1):
...
#		print "fdesc is ", fdesc
        ...
        # Check if we have byrefs or anything in the args which mean we still
        # need a translate.
        ...
        if len(bad_params) == 0 and len(retDesc) == 2 and retDesc[1] == 0:
            ...
            # else s remains None
        if s is None:
            s = '%s\treturn self._ApplyTypes_(%d, %s, %s, %s, %s, %s%s)' % (linePrefix, id, fdesc[
                4], retDesc, argsDesc, repr(name), resclsid, _BuildArgList(fdesc, names))
...
def _ResolveType(typerepr, itypeinfo):
    ...
    if isinstance(typerepr, tuple):
        ...
        if indir_vt == pythoncom.VT_PTR:
            ...
            # but (VT_PTR, (VT_USERDEFINED, otherhandle)) doesnt get the
            # indirection dropped.
            was_user = isinstance(subrepr, tuple) and subrepr[
...
def MakePublicAttributeName(className, is_global=False):
    ...
    return ''.join(
        [char for char in className if char in valid_identifier_chars])
...
def BuildCallList(fdesc, names, defNamedOptArg, defNamedNotOptArg,
                  defUnnamedArg, defOutArg, is_comment=False):
