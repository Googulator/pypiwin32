...
def __WrapDispatch(dispatch, userName=None, resultCLSID=None, typeinfo=None,
    ...
    return dynamic.Dispatch(
        dispatch, userName, WrapperClass, typeinfo, clsctx=clsctx)
...
def DispatchEx(clsid, machine=None, userName=None, resultCLSID=None,
               typeinfo=None, UnicodeToString=None, clsctx=None):
    ...
    # specifies otherwise, we exclude inproc apps when a remote machine is
    # used.
    if clsctx is None:
...
def DispatchWithEvents(clsid, user_event_class):
    ...
    * The user_event_class as passed to this function.
    ...
    # use 'type()' everywhere - revisit soon, as py2x will move to new-style
    # too...
    try:
    ...
    result_class = new_type(
        "COMEventClass", (disp_class, events_class, user_event_class), {
...
def Record(name, object):
    ...
    # thus, package may be module, or may be module's parent if demand
    # generated.
...
class DispatchBaseClass:
    ...
    def __repr__(self):
        ...
        return "<%s.%s instance at 0x%s>" % (
            mod_name, self.__class__.__name__, id(self))
...
def _get_good_object_(obj, obUserName=None, resultCLSID=None):
    if obj is None:
        ...
        return tuple(map(_get_good_object_, obj,
                         obUserNameTuple, resultCLSIDTuple))
    else:
        return _get_good_single_object_(obj, obUserName, resultCLSID)
...
class VARIANT(object):
    ...
    def __repr__(self):
        return "win32com.client.VARIANT(%r, %r)" % (
