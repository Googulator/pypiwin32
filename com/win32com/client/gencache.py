...
is_readonly = is_zip = hasattr(
    win32com,
    "__loader__") and hasattr(
        win32com.__loader__,
    "archive")
    ...
    if module is None:
        ...
        if is_readonly:
            ...
            for desc in GetGeneratedInfos():
                if key[0] == desc[0] and key[
                        1] == desc[1] and key[2] == desc[2]:
                    items.append(desc)
            if items:
...
def AddModuleToCache(typelibclsid, lcid, major, minor,
                     verbose=1, bFlushNow=not is_readonly):
...
def GetGeneratedInfos():
    ...
    if zip_pos >= 0:
        ...
        ret = []
        for file in files:
            if not os.path.isdir(file) and not os.path.splitext(file)[
                    1] == ".py":
                continue
