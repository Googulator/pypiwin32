...
def _GetGoodDispatchAndUserName(IDispatch, userName, clsctx):
    ...
    if userName is None:
        ...
        if isinstance(IDispatch, str):
            ...
            # 2to3 converts the above 'unicode' to 'str', but this will never
            # be executed in py3k
...
def _GetDescInvokeType(entry, invoke_type):
    ...
    if varkind == pythoncom.VAR_DISPATCH and invoke_type == pythoncom.INVOKE_PROPERTYGET:
        # DISPATCH_METHOD & DISPATCH_PROPERTYGET can be combined in
        # IDispatch::Invoke
...
def Dispatch(IDispatch, userName=None, createClass=None, typeinfo=None,
...
def MakeOleRepr(IDispatch, typeinfo, typecomp):
    ...
    if typeinfo is not None:
        try:
            ...
            # a DISPATCH typeinfo.
            if attr[5] == pythoncom.TKIND_INTERFACE and attr[
...
def DumbDispatch(IDispatch, userName=None, createClass=None,
                 UnicodeToString=None, clsctx=pythoncom.CLSCTX_SERVER):
...
class CDispatch:
    ...
    def __init__(self, IDispatch, olerepr, userName=None,
                 UnicodeToString=None, lazydata=None):
        assert UnicodeToString is None, "this is deprecated and will go away"
        if userName is None:
            userName = "<unknown>"
    ...
    def __call__(self, *args):
        ...
        if invkind is not None:
            ...
            return self._get_good_object_(self._oleobj_.Invoke(
                *allArgs), self._olerepr_.defaultDispatchName, None)
        raise TypeError(
            "This dispatch object does not define a default method")

    def __bool__(self):
        # ie "if object:" should always be "true" - without this, __len__ is
        # tried.
        return True
    ...
    def _ApplyTypes_(self, dispid, wFlags, retType,
    ...
    def _wrap_dispatch_(self, ob, userName=None,
                        returnCLSID=None, UnicodeToString=None):
    ...
    def _get_good_single_object_(self, ob, userName=None, ReturnCLSID=None):
        ...
        if isinstance(ob, PyIUnknownType):
            try:
                ...
                # It is an IUnknown, but not an IDispatch, so just let it
                # through.
                return ob
            return self._wrap_dispatch_(ob, userName, ReturnCLSID)
        return ob
    ...
    def _get_good_object_(self, ob, userName=None, ReturnCLSID=None):
        ...
        if ob is None:  # Quick exit!
            ...
            return tuple(map(lambda o, s=self, oun=userName,
                             rc=ReturnCLSID: s._get_good_single_object_(o, oun, rc), ob))
        else:
    ...
    def _proc_(self, name, *args):
        ...
        try:
            ...
            dispId = item.dispid
            return self._get_good_object_(self._oleobj_.Invoke(
                *(dispId, LCID, item.desc[4], 0) + (args)))
        except KeyError:
    ...
    def __LazyMap__(self, attr):
        try:
            if self._LazyAddAttr_(attr):
                debug_attr_print(
                    "%s.__LazyMap__(%s) added something" %
    ...
    def __getattr__(self, attr):
        ...
        if not retEntry is None:
            ...
            debug_attr_print(
                "Getting property Id 0x%x from OLE object" %
                retEntry.dispid)
            try:
