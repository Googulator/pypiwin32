...
def WriteSinkEventMap(obj, stream):
    ...
    for name, entry in obj.propMapGet.items() + obj.propMapPut.items() + \
            obj.mapFuncs.items():
        fdesc = entry.desc
        print >> stream, '\t\t%9d : "%s",' % (
        ...
        if self.aliasDoc:
            ...
            if isinstance(ai, type(0)):
                try:
                    ...
                    print >> stream, self.doc[
                        0] + " = None # Can't convert alias info " + str(ai)
        print >> stream
        self.bWritten = 1
...
class DispatchItem(build.DispatchItem, WritableItem):
    ...
    def WriteCallbackClassBody(self, generator):
        ...
        for name, entry in self.propMapGet.items() + self.propMapPut.items() + \
                self.mapFuncs.items():
            fdesc = entry.desc
...
class Generator:
    ...
    def do_generate(self):
        ...
        if self.generate_type == GEN_FULL:
            items = sorted([l for l in oleItems.itervalues() if l is not None])
            for oleitem in items:
                self.progress.Tick()
