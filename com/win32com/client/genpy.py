...
##    WriteAliasesForItem(self, aliasItems)
    ...
    def WriteEnumerationItems(self, stream):
        ...
        names = sorted(self.mapVars.keys())
        for name in names:
            ...
            if vdesc[4] == pythoncom.VAR_CONST:
                ...
                try:
                    ...
                    # Sanitize it, in case the repr contains its own quotes.
                    # (??? line breaks too ???)
                    use = use.replace('"', "'")
...
class VTableItem(build.VTableItem, WritableItem):
    ...
    def WriteVTableMap(self, generator):
        ...
        print("%s_vtables_dispatch_ = %d" % (
            self.python_name, self.bIsDispatch), file=stream)
...
class DispatchItem(build.DispatchItem, WritableItem):
    ...
    def WriteClass(self, generator):
        ...
        # should probably rethink this (ie, maybe write both sides for sinks,
        # etc)
        if self.bIsSink:
    ...
    def WriteClassHeader(self, generator):
        ...
        try:
            ...
            print("\t# This class is creatable by the name '%s'" % (
                progId), file=stream)
    ...
    def WriteClassBody(self, generator):
        ...
        names = sorted(self.mapFuncs.keys())
        ...
        for key in names:
            ...
            if generator.bBuildHidden or not entry.hidden:
                ...
                if resultName:
                    print("\t\t# Property '%s' is an object of type '%s'" % (
                        key, resultName), file=stream)
                lkey = key.lower()
                details = entry.desc
                ...
                print('\t\t"%s": %s,' % (
                    build.MakePublicAttributeName(key), mapEntry), file=stream)
        ...
        for key in names:
            ...
            if generator.bBuildHidden or not entry.hidden:
                if entry.GetResultName():
                    print("\t\t# Method '%s' returns object of type '%s'" % (
                        key, entry.GetResultName()), file=stream)
                details = entry.desc
                lkey = key.lower()
                argDesc = details[2]
        ...
        if specialItems["value"]:
            ...
            if propArgs is None:
                ...
                ret = [
                    ...
                    propArgs]
            print("\t# Default %s for this class is '%s'" % (
                typename, entry.names[0]), file=stream)
            for line in ret:
                print(line, file=stream)
        ...
        # __getitem__ iteration to fail for index 0 where the dynamic iteration
        # succeeds.
        if specialItems["_newenum"]:
            ...
            resultCLSID = "None"
        # If we dont have a good CLSID for the enum result, assume it is the
        # same as the Item() method.
...
class CoClassItem(build.OleItem, WritableItem):
    ...
    def WriteClass(self, generator):
        ...
        try:
            progId = pythoncom.ProgIDFromCLSID(self.clsid)
            print("# This CoClass is known by the name '%s'" % (
                progId), file=stream)
        except pythoncom.com_error:
            pass
        print('class %s(CoClassBaseClass): # A CoClass' % (
            self.python_name), file=stream)
        if doc and doc[1]:
...
class Generator:
    ...
    def __init__(self, typelib, sourceFilename, progressObject,
                 bBuildHidden=1, bUnicodeToString=None):
    ...
    def _Build_CoClassChildren(
            self, coclass, coclass_info, oleItems, vtableItems):
        sources = {}
        interfaces = {}
        for info, info_type, refType, doc, refAttr, flags in coclass_info:
            ...
            if refAttr.typekind == pythoncom.TKIND_DISPATCH or (
                ...
                if clsid not in vtableItems and refAttr[
                        11] & pythoncom.TYPEFLAG_FDUAL:
    ...
    def do_generate(self):
        ...
        if self.generate_type == GEN_FULL:
            items = sorted([l for l in oleItems.values() if l is not None])
            for oleitem in items:
                self.progress.Tick()
        ...
        if self.generate_type == GEN_FULL:
            ...
            for item in vtableItems.values():
                print("\t'%s' : '%s'," % (
                    item.clsid, item.python_name), file=stream)
            print('}', file=stream)
            print(file=stream)
            ...
            for item in oleItems.values():
                if item is not None:
                    print("\t'%s' : %s," % (
                        str(item.clsid), repr(item.python_name)), file=stream)
            print('}', file=stream)
            print("VTablesToClassMap = {}", file=stream)
            print("VTablesToPackageMap = {", file=stream)
            for item in vtableItems.values():
        ...
        print(file=stream)
        # Bit of a hack - build a temp map of iteItems + vtableItems -
        # coClasses
        map = {}
        for item in oleItems.values():
            ...
            for type_info_tuple in infos:
                ...
                if infotype == pythoncom.TKIND_COCLASS:
                    ...
                    if found:
                        ...
                        break
            if not found:
                # Doesn't appear in a class defn - look in the interface
                # objects for it
                for type_info_tuple in infos:
                    info, infotype, doc, attr = type_info_tuple
                    if infotype in [pythoncom.TKIND_INTERFACE,
    ...
    def do_gen_child_item(self, oleitem):
        ...
        oleitem.WriteClass(self)
        if oleitem.bWritten:
            print('win32com.client.CLSIDToClass.RegisterCLSID( "%s", %s )' % (
                oleitem.clsid, oleitem.python_name), file=self.file)
