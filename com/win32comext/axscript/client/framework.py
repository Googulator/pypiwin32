...
def MakeValidSysOuts():
    if not isinstance(sys.stdout, SafeOutput):
        ...
        # If someone else has already redirected, then assume they know what
        # they are doing!
...
class AXScriptCodeBlock:
    ...
    def __init__(self, name, codeText, sourceContextCookie,
...
class EventSink:
    ...
    def _invoke_(self, dispid, lcid, wFlags, args):
        ...
        # print "Invoke for ", event, "on", self.myScriptItem, " - calling",
        # self.myInvokeMethod
        return self.myInvokeMethod(
            self.myScriptItem, event, lcid, wFlags, args)
...
#		trace("Creating ScriptItem", name, "of parent", parentItem,"with dispatch", dispatch)
    ...
    def __repr__(self):
        ...
        if self.flags is not None and self.flags & axscript.SCRIPTITEM_GLOBALMEMBERS:
            flagsDesc = "/Global"
        return "<%s at %d: %s%s>" % (
            self.__class__.__name__, id(self), self.name, flagsDesc)
...
#			print "**** Made dispatch"
    ...
    def IsVisible(self):
        return (self.flags & (axscript.SCRIPTITEM_ISVISIBLE |
                              axscript.SCRIPTITEM_ISSOURCE)) != 0
...
#		trace("BuildEvents for named item", self._GetFullItemName())
            ...
            while True:
                ...
                if isSubObject:
                    try:
                        ...
                        # As use in IE is deprecated, I am not solving this
                        # now.
                        if isinstance(result, pythoncom.TypeIIDs[
                                      pythoncom.IID_IDispatch]):
                            name = names[0]
                            subObj = self.GetCreateSubItem(
                                self, name, result, axscript.SCRIPTITEM_ISVISIBLE)
                            # print "subobj", name, "flags are", subObj.flags,
                            # "mydisp=", self.dispatch, "result disp=", result,
                            # "compare=", self.dispatch==result
                            subObj.BuildEvents()
                            subObj.Register()
                    except pythoncom.com_error:
                        pass
    ...
    def GetDefaultSourceTypeInfo(self, typeinfo):
        ...
        for i in range(cImplType):
            ...
            # that isn't marked as restricted.
            flags = typeinfo.GetImplTypeFlags(i)
            if (flags & (pythoncom.IMPLTYPEFLAG_FDEFAULT | pythoncom.IMPLTYPEFLAG_FSOURCE |
                         pythoncom.IMPLTYPEFLAG_FRESTRICTED)) == pythoncom.IMPLTYPEFLAG_FDEFAULT:
                # Get the handle to the implemented interface.
                href = typeinfo.GetRefTypeOfImplType(i)
...
#		self.DoAddScriptlet(None, code, itemName, subItemName, eventName, delimiter,sourceContextCookie, startLineNumber)
    ...
    def SetScriptState(self, state):
        # print "SetScriptState with %s - currentstate = %s" %
        # (state_map.get(state),state_map.get(self.scriptState))
...
#			RaiseAssert(scode=winerror.E_FAIL, desc="Unknown safety options")
    ...
    def CheckConnectedOrDisconnected(self):
        ...
        RaiseAssert(
            winerror.E_UNEXPECTED,
            "Not connected or disconnected - %d" %
            self.scriptState)
...
#		self._DumpNamedItems_()
    ...
    def ChangeScriptState(self, state):
        # print "  ChangeScriptState with %s - currentstate = %s" %
        # (state_map.get(state),state_map.get(self.scriptState))
    ...
    # This stack frame is debugged - therefore we do as little as possible in
    # it.
    ...
    def _EvalInScriptedSection(self, codeBlock, globals, locals=None):
        if self.debugManager:
            self.debugManager.OnEnterScript()
            if self.debugManager.adb.appDebugger:
                return self.debugManager.adb.runeval(
                    codeBlock, globals, locals)
            else:
    ...
    def HandleException(self, codeBlock):
        ...
        if result_exception is not None:
            ...
            # reset ourselves to 'connected' so further events continue to
            # fire.
