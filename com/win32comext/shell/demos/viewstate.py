...
# This may blow up if folder has never been opened in Explorer and has no
# ColInfo yet
...
def update_colinfo(not_used, dir_name, fnames):
    for fname in fnames:
        ...
        if os.path.isdir(full_fname):
            ...
            # not all folders already have column info, and we're replacing it
            # anyway
            ...
            s = None
            # attribute names read from registry, can't find any way to
            # enumerate IPropertyBag
            for attr in ('Address', 'Buttons', 'Col', 'Vid', 'WFlags', 'FFlags',
                         'Sort', 'SortDir', 'ShowCmd', 'FolderType', 'Mode', 'Rev'):
