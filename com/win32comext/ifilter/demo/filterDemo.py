            ...
            try:
                ...
                while True:
                    try:
                        ...
                        # A zero value signifies character-by-character
                        # correspondence between the source text and the
                        # derived text.
                        ...
                        # the set guid and property id. (note: the id can be a
                        # number or a string.
                        ...
                        if e[0] == FILTER_E_END_OF_CHUNKS:
                            ...
                            # fail as some filters (ie. the Msoft office ones
                            # can get stuck here)
                            errCnt += 1
                            if errCnt > maxErrors:
                                raise
                            ...
                            # any other type of error really can't be recovered
                            # from
                            raise

                    # reset consecutive errors (some filters may get stuck in a
                    # lopp if embedding or link failures occurs
                    errCnt = 0

                    if flags == CHUNK_TEXT:
                        # its a text segment - get all available text for this
                        # chunk.
    ...
    def _bind_to_filter(self, fileName):
        ...
        if pythoncom.StgIsStorageFile(fileName):
            ...
            try:
                ...
                # 0x80004002: # no interface, try the load interface (this
                # happens for some MSoft files)
                if e[0] == -2147467262:
    ...
    def _get_text(self, body_chunks):
        ...
        while True:
            try:
                ...
                if e[0] in [FILTER_E_NO_MORE_TEXT,
                            FILTER_E_NO_MORE_TEXT, FILTER_E_NO_TEXT]:
                    break
                else:
