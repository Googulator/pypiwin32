...
def IsPackageDir(path, packageName, knownFileName):
    ...
    if knownFileName is None:
        knownFileName = "."
    return FileExists(os.path.join(
        os.path.join(path, packageName), knownFileName))
...
def FindAppPath(appName, knownFileName, searchPaths):
    ...
    for pathLook in searchPaths:
        if FileExists(os.path.join(pathLook, knownFileName)):
            ...
            return os.path.abspath(pathLook)
    raise error(
        "The file %s can not be located for application %s" %
...
def LocatePath(fileName, searchPaths):
    ...
    """
    import os
    return os.path.abspath(os.path.split(
        LocateFileName(fileName, searchPaths))[0])
...
def FindRegisterPackage(packageName, knownFile,
                        searchPaths, registryAppName=None):
...
def RegisterShellInfo(searchPaths):
    ...
    regutil.RegisterShellCommand(
        "Open",
        QuotedFileName(exePath) +
        " \"%1\" %*",
        "&Run")
    regutil.SetRegistryDefaultValue(
        "Python.File\\DefaultIcon", "%s,0" %
        exePath, win32con.HKEY_CLASSES_ROOT)
