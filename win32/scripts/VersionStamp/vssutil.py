...
def SubstituteInString(inString, evalEnv):
    ...
    for i in range(len(fields)):
        ...
        if not didSubst:
            newFields.append(strVal)
    return string.join(list(map(str, newFields)), "")
...
def MakeNewBuildNo(project, buildDesc=None, auto=0, bRebrand=0):
    ...
    if num > 0:
        ...
        if win32ui.MessageBox(
                msg, project, win32con.MB_YESNO) != win32con.IDYES:
            return
    ...
    if buildNo is None:
        ...
        try:
            ...
            raise error(
                "The previous label could not be incremented: %s" %
                (oldBuild))

    if not auto:
        from pywin.mfc import dialog
        buildNo = dialog.GetSimpleInput(
            "Enter new build number", buildNo, "%s - Prev: %s" %
            (project, oldBuild))
        if buildNo is None:
            return
...
if __name__ == '__main__':
    ...
    SubstituteVSSInFile(
        tp,
        "d:\\src\\pythonex\\win32\\win32.txt",
        "d:\\temp\\win32.txt")
