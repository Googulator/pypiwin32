...
class TestOverlapped(unittest.TestCase):
    ...
    def testCompletionPortsMultiple(self):
        ...
        for PORT in range(9123, 9125):
            ...
            sock.listen(1)
            socks.append(sock)
            new = win32file.CreateIoCompletionPort(
                sock.fileno(), ioport, PORT, 0)
    ...
    def testCompletionPortsNonQueued(self, test_overlapped_death=0):
        ...
        t = threading.Thread(
            target=self._IOCPServerThread, args=(
                handle, port, test_overlapped_death))
...
class TestSocketExtensions(unittest.TestCase):
    ...
    def acceptWorker(self, port, running_event, stopped_event):
        ...
        nbytes = win32file.GetOverlappedResult(
            listener.fileno(), overlapped, False)
    ...
    def testAcceptEx(self):
        ...
        t = threading.Thread(
            target=self.acceptWorker, args=(
                port, running, stopped))
        t.start()
        running.wait(2)
...
class TestConnect(unittest.TestCase):
    ...
    def test_connect_with_payload(self):
        ...
        try:
            win32file.ConnectEx(
                s2, self.addr, ol, str2bytes("some expected request"))
...
class TestTransmit(unittest.TestCase):
    ...
    def test_transmit(self):
        ...
        win32file.TransmitFile(
            s2, win32file._get_osfhandle(
                f.fileno()), val_length, 0, ol, 0)
        length += win32file.GetOverlappedResult(s2.fileno(), ol, 1)
        ...
        win32file.TransmitFile(
            s2, win32file._get_osfhandle(
                f.fileno()), val_length, 0, ol, 0, aaa, bbb)
        length += win32file.GetOverlappedResult(s2.fileno(), ol, 1)

        ol = pywintypes.OVERLAPPED()
        f.seek(0)
        win32file.TransmitFile(s2, win32file._get_osfhandle(
            f.fileno()), val_length, 0, ol, 0, empty, empty)
        length += win32file.GetOverlappedResult(s2.fileno(), ol, 1)
        ...
        win32file.TransmitFile(
            s2, win32file._get_osfhandle(
                f.fileno()), val_length, 0, ol, 0, None, ccc)
        length += win32file.GetOverlappedResult(s2.fileno(), ol, 1)
        ...
        win32file.TransmitFile(
            s2, win32file._get_osfhandle(
                f.fileno()), val_length, 0, ol, 0, ddd)
        length += win32file.GetOverlappedResult(s2.fileno(), ol, 1)
