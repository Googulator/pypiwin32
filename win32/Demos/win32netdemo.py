...
def UserEnum():
    ...
    while True:
        ...
        verbose(
            "Call to NetUserEnum obtained %d entries of %d total" %
            (len(data), total))
        for user in data:
...
# print "Call to NetGroupEnum obtained %d entries of %d total" %
# (len(data), total)
        for group in data:
        ...
        for group in data:
            ...
            while True:
                ...
                for member in memberdata:
                    ...
                    username, domain, type = win32security.LookupAccountSid(server, member[
                                                                            'sid'])
                    nmembers = nmembers + 1
                    verbose(
                        " Member %s (%s)" %
...
def SetInfo(userName=None):
    ...
    try:
        ...
        if str(new) != "Test comment":
            raise RuntimeError(
                "Could not read the same comment back - got %s" %
                new)
