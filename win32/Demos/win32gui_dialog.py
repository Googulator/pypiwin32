    ...
    def __init__(self, **kw):
        ...
        for name, val in kw.items():
            if name not in self.__dict__:
                raise ValueError(
                    "LVITEM structures do not have an item '%s'" %
                    (name,))
...
class DemoWindowBase:
    ...
    def _RegisterWndClass(self):
        ...
        try:
            wc.hIcon = win32gui.LoadIcon(
                this_app, 1)  # python.exe and pythonw.exe
    ...
    def _GetDialogTemplate(self, dlgClassName):
        ...
        dlg.append([130, "Enter something", -
                    1, (5, 5, 200, 9), cs | win32con.SS_LEFT])
...
##        dlg.append(['SysListView32', "Title", IDC_LISTBOX, (5, 505, 200, 200), s])
    ...
    def _SetupList(self):
        ...
        win32gui.SendMessage(
            self.hwndList,
            commctrl.LVM_SETEXTENDEDLISTVIEWSTYLE,
            0,
            child_ex_style)
    ...
    def OnCommand(self, hwnd, msg, wparam, lparam):
        ...
        if id == IDC_BUTTON_SEARCH:
            ...
            thread = threading.Thread(
                target=fill_slowly, args=(
                    self.result_queue, self.hwnd))
            thread.start()
    ...
    # These function differ based on how the window is used, so may be
    # overridden
