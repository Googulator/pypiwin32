...
def PackLVITEM(item=None, subItem=None, state=None, stateMask=None,
               text=None, image=None, param=None, indent=None):
    extra = []  # objects we must keep references to
    mask = 0
...
def PackLVCOLUMN(fmt=None, cx=None, text=None,
                 subItem=None, image=None, order=None):
    extra = []  # objects we must keep references to
    mask = 0
...
def PackDEV_BROADCAST(devicetype, rest_fmt, rest_data,
                      extra_data=_make_bytes('')):
...
def PackDEV_BROADCAST_HANDLE(handle, hdevnotify=0, guid=_make_bytes(
        "\0" * 16), name_offset=0, data=_make_bytes("\0")):
    return PackDEV_BROADCAST(win32con.DBT_DEVTYP_HANDLE, "PP16sl",
                             (int(handle), int(hdevnotify),
...
def PackDEV_BROADCAST_DEVICEINTERFACE(classguid, name=""):
    ...
    return PackDEV_BROADCAST(
        win32con.DBT_DEVTYP_DEVICEINTERFACE, rest_fmt, rest_data)
