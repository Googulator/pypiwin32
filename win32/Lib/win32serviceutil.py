...
def InstallPerfmonForService(serviceName, iniName, dllName=None):
    ...
    # Still not found - look for the standard one in the same dir as
    # win32service.pyd
    if not dllName:
        try:
            tryName = os.path.join(
                os.path.split(
                    win32service.__file__)[0],
                "perfmondata.dll")
    ...
    try:
        ...
        try:
            win32api.RegSetValueEx(
                subKey, "Library", 0, win32con.REG_SZ, dllName)
            win32api.RegSetValueEx(
                subKey,
                "Open",
                0,
    ...
    try:
        ...
        if delayedstart is not None:
            try:
                ...
                # delayed start only exists on Vista and later - warn only when
                # trying to set delayed to True
                if delayedstart:
...
def GetServiceClassString(cls, argv=None):
    ...
    if modName == '__main__':
        try:
            ...
            raise error(
                "Could not resolve the path name '%s' to a full path" %
                (argv[0]))
...
def HandleCommandLine(cls, serviceClassString=None, argv=None,
...
class ServiceFramework:
    ...
    def ReportServiceStatus(self, serviceStatus,
                            waitHint=5000, win32ExitCode=0, svcExitCode=0):
        ...
        if serviceStatus in [win32service.SERVICE_RUNNING,
