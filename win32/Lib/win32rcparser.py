...
class RCParser:
    ...
    def addId(self, id_name):
        if id_name in self.ids:
            ...
            # IDOK, IDCANCEL etc are special - if a real resource has this
            # value
    ...
    def lang(self):
        while self.token[0:4] == "LANG" or self.token[
                0:7] == "SUBLANG" or self.token == ',':
            self.getToken()
    ...
    def parse_dialog(self, name):
        ...
        self.getToken()
        while not (self.token is None or self.token ==
                   "" or self.token == "END"):
            if self.token == "STYLE":
    ...
    def styles(self, defaults, defaultStyle):
        ...
        i = 0
        Not = False
        while ((i % 2 == 1 and ("|" == self.token or "NOT" == self.token))
               or (i % 2 == 0)) and not self.token is None:
            Not = False
            if "NOT" == self.token:
                Not = True
                self.getToken()
