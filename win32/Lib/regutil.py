...
def SetRegistryDefaultValue(subKey, value, rootkey=None):
    ...
    if isinstance(value, str):
        ...
        raise TypeError(
            "Value must be string or integer - was passed " +
            repr(value))
...
def GetRegisteredExe(exeAlias):
    ...
    return win32api.RegQueryValue(
        GetRootKey(), GetAppPathsKey() + "\\" + exeAlias)
...
def GetRegisteredHelpFile(helpDesc):
    ...
    try:
        ...
        try:
            return GetRegistryDefaultValue(BuildDefaultPythonKey(
            ) + "\\Help\\" + helpDesc, win32con.HKEY_CURRENT_USER)
        except win32api.error:
            pass
    return None
