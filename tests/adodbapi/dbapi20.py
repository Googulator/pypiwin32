...
# Add a literal string to the format insertion test to catch trivial
# re-format algorithms

# Revision 1.13  2013/05/08 14:31:50  kf7xm
# Quick switch to Turn off IDEMPOTENT_CLOSE test. Also: Silence teardown
# failure
...
class DatabaseAPI20Test(unittest.TestCase):
    ...
    def test_Exceptions(self):
        ...
        # defined heirarchy.
        if sys.version[
            0] == '3':  # under Python 3 StardardError no longer exists
    ...
    def test_rowcount(self):
        ...
        try:
            ...
            self.executeDDL2(cur)
            assert cur.rowcount == -1, 'cursor.rowcount not being reset to -1 after executing ' \
    ...
    def test_fetchmany(self):
        ...
        try:
            ...
            cur.execute('select name from %sbooze' % self.table_prefix)
            r = cur.fetchmany()
            assert len(r) == 1, 'cursor.fetchmany retrieved incorrect number of rows, ' \
            ...
            assert len(rows) == 0, 'cursor.fetchmany should return an empty sequence if ' \
            ...
            assert len(r) == 0, 'cursor.fetchmany should return an empty sequence if ' \
    ...
    def test_None(self):
        ...
        try:
            ...
            self.executeDDL1(cur)
            cur.execute(
                'insert into %sbooze values (NULL)' %
                self.table_prefix)
            cur.execute('select name from %sbooze' % self.table_prefix)
            r = cur.fetchall()
            assert len(r) == 1
