...
def _DoTestCollection(vbtest, col_name, expected):
    ...
    if check != list(expected):
        raise error(
            "Collection %s didn't have %r (had %r)" %
...
def TestArrays(vbtest, bUseGenerated):
    ...
    try:
        ...
        assert exc.excepinfo[
                   1] == "Python COM Server Internal Error", "Didnt get the correct exception - '%s'" % (exc,)
...
def TestStructs(vbtest):
    ...
    if s.sub_val.array_val[0].int_val != 0 or str(
            s.sub_val.array_val[0].str_val) != "zero":
    ...
    try:
        ...
        pass
    m = s.__members__
    assert m[0] == "int_val" and m[1] == "str_val" and m[
                                                           2] == "ob_val" and m[3] == "sub_val", m
