...
class HierFrameItem(HierListItem):
    ...
    def GetText(self):
        ...
        return "%s   (%s:%d)" % (name, os.path.split(
            self.myobject.f_code.co_filename)[1], self.myobject.f_lineno)
...
class Debugger(debugger_parent):
    ...
    def close(self, frameShutdown=0):
        ...
        # Hide the debuger toolbars (as they wont normally form part of the
        # main toolbar state.
        for id, klass, float in DebuggerDialogInfos:
            try:
    ...
    def set_break(self, filename, lineno, temporary=0, cond=None):
        ...
        return debugger_parent.set_break(
            self, filename, lineno, temporary, cond)
    ...
    def user_exception(self, frame, exc_info):
        ...
        if self.get_option(OPT_STOP_EXCEPTIONS):
            ...
            if sys.version_info > (3,) and not isinstance(
    ...
    def GUICheckInit(self):
        ...
        for id, klass, float in DebuggerDialogInfos:
            ...
            # This would be better done using a CDockState, but that class is
            # not wrapped yet
...
#			doc.MarkerAdd(self.shownLineCallstack[1], MARKER_CURRENT)
    ...
    def ShowLineNo(self, filename, lineno):
        ...
        if os.path.isfile(filename) and scriptutils.JumpToDocument(
                filename, lineno):
            if not wasOpen:
                doc = editor.editorTemplate.FindOpenDocument(filename)
                if doc is not None:
