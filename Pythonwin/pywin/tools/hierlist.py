...
class HierList(object.Object):
    ...
    def __init__(self, root, bitmapID=win32ui.IDB_HIERFOLDERS,
                 listBoxId=None, bitmapMask=None):
...
#		self.list.AttachObject(self)
        ...
        if sys.version_info[0] < 3:
            parent.HookNotify(
                self.OnTreeItemExpanding,
                commctrl.TVN_ITEMEXPANDINGA)
            parent.HookNotify(
                self.OnTreeItemSelChanged,
    ...
    def Refresh(self, hparent=None):
        ...
        if hparent not in self.filledItemHandlesMap:
            # This item has never been expanded, so no refresh can possibly be
            # required.
            return
