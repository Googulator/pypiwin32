...
class HLIPythonObject(hierlist.HierListItem):
    ...
    def __repr__(self):
        try:
            ...
            type = "Generic"
        return "HLIPythonObject(" + type + ") - name: " + \
            self.name + " object: " + repr(self.myobject)
...
class HLICode(HLIPythonObject):
    ...
    def GetSubList(self):
        ...
        ret.append(
            MakeHLI(
                self.myobject.co_filename,
                "Filename (co_filename)"))
        ret.append(
            MakeHLI(
                self.myobject.co_argcount,
                "Number of args (co_argcount)"))
        ret.append(
            MakeHLI(
                self.myobject.co_varnames,
...
class HLIDict(HLIPythonObject):
    ...
    def GetSubList(self):
        ret = []
        keys = sorted(self.myobject.keys())
        for key in keys:
...
# In Python 1.6, strings and Unicode have builtin methods, but we dont
