...
class MainFrame(app.MainFrame):
    ...
    def OnCommand(self, wparam, lparam):
        ...
        # as its parent, and assume it must be a docked view (which it will in
        # an MDI app)
        try:
            # Raise an exception if none - good - then we want default handling
            v = self.GetActiveView()
            # Main frame _does_ have a current view (ie, a docking view) - see
            # if it wants it.
...
class InteractivePythonApp(app.CApp):
    ...
    def HookCommands(self):
        ...
        self.HookCommandUpdate(
            self.OnUpdateInteractiveWindow,
    ...
    def InitInstance(self):
        ...
        # By now all these modules should be loaded, so all the toolbar IDs
        # loaded.
        try:
        ...
        try:
            ...
            if argType == "/edit":
                ...
                if dde:
                    ...
                    dde.Exec(
                        "win32ui.GetApp().OpenDocumentFile(%s)" %
                        (repr(fname)))
                else:
                ...
                if dde:
                    dde.Exec(
                        "from pywin.framework import scriptutils;scriptutils.RunScript('%s', '%s', 1)" %
                        (args[argStart],
                         ' '.join(
                            args[
                                argStart +
                ...
                if dde:
                    dde.Exec(
                        "from pywin.framework import scriptutils;scriptutils.RunScript('%s', '%s', 0)" %
                        (args[argStart],
                         ' '.join(
                            args[
                                argStart +
