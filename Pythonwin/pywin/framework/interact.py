...
#		self.SetWordWrap(win32ui.CRichEditView_WrapNone)
    ...
    def HookHandlers(self):
        # Hook menu command (executed when a menu item with that ID is selected
        # from a menu/toolbar
    ...
    def GetBlockBoundary(self, lineNo):
        ...
        if prefix is None:        # Non code block
            ...
            startLineNo = lineNo
            while startLineNo > 0:
                if GetPromptPrefix(self.DoGetLine(
                        startLineNo - 1)) is not None:
    ...
    def ProcessEnterEvent(self, event):
        # If autocompletion has been triggered, complete and do not process
        # event
        ...
        # If we are not in a code block just go to the prompt (or create a new
        # one)
        if not isCode:
            self.AppendToPrompt([])
            win32ui.SetStatusText(
                win32ui.LoadString(
                    afxres.AFX_IDS_IDLEMESSAGE))
            return
...
#					win32ui.SetStatusText('Successfully executed statement')
        ...
        if bNeedIndent:
            ...
            if _is_block_opener(curLine):
                ...
                indent = indent[:-1]
            # use ReplaceSel to ensure it goes at the cursor rather than end of
            # buffer.
...
class DockedInteractiveView(DockedInteractiveViewParent):
    ...
    def OnKillFocus(self, msg):
        # If we are losing focus to another in this app, reset the main frame's
        # active view.
        hwnd = wparam = msg[2]
        try:
...
def CloseInteractiveWindow():
    ...
    if edit is not None and edit.currentView is not None:
        if edit.currentView.GetParentFrame() == win32ui.GetMainFrame():
            ...
            # It is a standard window - destroy the frame/view, allowing the
            # object itself to remain.
