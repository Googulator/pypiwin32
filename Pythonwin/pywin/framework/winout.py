...
# other than the main thread attempts to print, it is always queued until
# next idle time
...
class WindowOutputFrame(window.MDIChildWnd):
    ...
    def PreCreateWindow(self, cc):
        ...
        if self.template.defSize and self.template.defSize[
                0] != self.template.defSize[1]:
...
class WindowOutputViewImpl:
    ...
    def OnRClick(self, params):
        ...
        for appendParams in paramsList:
            if not isinstance(appendParams, type(())):
...
class WindowOutputViewScintilla(
    ...
    def HookHandlers(self):
        ...
        self.GetParent().HookNotify(
            self.OnScintillaDoubleClick,
            scintillacon.SCN_DOUBLECLICK)
# self.HookMessage(self.OnLDoubleClick,win32con.WM_LBUTTONDBLCLK)
...
class WindowOutput(docview.DocTemplate):
    ...
    def QueueIdleHandler(self, handler, count):
        try:
            ...
            # If the queue is empty, then we are back to idle and restart
            # interrupt logic.
            if bEmpty:
                self.interruptCount = 0
    ...
    # Returns true if the Window is OK (either cos it was, or because it was
    # recreated
...
#			debug("not my thread - ignoring queue options!\n")
        ...
        try:
            ...
            # This can happen as the app is shutting down, so we send it to the
            # C++ debugger
