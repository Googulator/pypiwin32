...
class SyntEditView(SyntEditViewParent):
    ...
    def OnInitialUpdate(self):
        ...
        self.HookCommand(
            self.OnCmdViewFoldTopLevel,
            win32ui.ID_VIEW_FOLD_TOPLEVEL)

        # Define the markers
#		self.SCIMarkerDeleteAll()
        self.SCIMarkerDefineAll(
            MARKER_BOOKMARK, scintillacon.SC_MARK_ROUNDRECT, win32api.RGB(
                0x0, 0x0, 0x0), win32api.RGB(
                0, 0xff, 0xff))
        ...
        if 1:  # traditional markers
            self.SCIMarkerDefineAll(
                scintillacon.SC_MARKNUM_FOLDEROPEN, scintillacon.SC_MARK_MINUS, win32api.RGB(
                    0xff, 0xff, 0xff), win32api.RGB(
                    0, 0, 0))
            self.SCIMarkerDefineAll(
                scintillacon.SC_MARKNUM_FOLDER, scintillacon.SC_MARK_PLUS, win32api.RGB(
                    0xff, 0xff, 0xff), win32api.RGB(
                    0, 0, 0))
            self.SCIMarkerDefineAll(
                scintillacon.SC_MARKNUM_FOLDERSUB, scintillacon.SC_MARK_EMPTY, win32api.RGB(
                    0xff, 0xff, 0xff), win32api.RGB(
                    0, 0, 0))
            self.SCIMarkerDefineAll(
                scintillacon.SC_MARKNUM_FOLDERTAIL, scintillacon.SC_MARK_EMPTY, win32api.RGB(
                    0xff, 0xff, 0xff), win32api.RGB(
                    0, 0, 0))
            self.SCIMarkerDefineAll(
                scintillacon.SC_MARKNUM_FOLDEREND, scintillacon.SC_MARK_EMPTY, win32api.RGB(
                    0xff, 0xff, 0xff), win32api.RGB(
                    0, 0, 0))
            self.SCIMarkerDefineAll(
            ...
            self.SCIMarkerDefineAll(
                ...
                win32api.RGB(
                    ...
                    0))
            self.SCIMarkerDefineAll(
                scintillacon.SC_MARKNUM_FOLDER, scintillacon.SC_MARK_CIRCLEPLUS, win32api.RGB(
                    0xff, 0xff, 0xff), win32api.RGB(
                    0, 0, 0))
            self.SCIMarkerDefineAll(
                scintillacon.SC_MARKNUM_FOLDERSUB, scintillacon.SC_MARK_VLINE, win32api.RGB(
                    0xff, 0xff, 0xff), win32api.RGB(
                    0, 0, 0))
            self.SCIMarkerDefineAll(
    ...
    def DoConfigChange(self):
        ...
        fold_flags = 0
        self.SendScintilla(
            scintillacon.SCI_SETMODEVENTMASK,
        ...
        if bSmartTabs:
            ...
            if ext.indentwidth == 5:
                # Either 5 literal spaces, or a single tab character. Assume a
                # tab
                usetabs = 1
                indentwidth = tabSize
            else:
    ...
    def OnDebuggerStateChange(self, state):
        if state == dbgcon.DBGSTATE_NOT_DEBUGGING:
            ...
            # Not quite white - useful when no marker margin, so set as
            # background color.
            self.SCIMarkerSetBack(
                MARKER_BREAKPOINT, win32api.RGB(
                    0xef, 0xef, 0xef))
        else:
    ...
    def HookDocumentHandlers(self):
        ...
        self.HookMessage(
            self.OnCheckExternalDocumentUpdated,
...
#			max_line = self.GetLineCount()
            if self.SCIGetFoldLevel(
                    line_click) & scintillacon.SC_FOLDLEVELHEADERFLAG:
                # If a fold point.
                self.SCIToggleFold(line_click)
        return 1

    def OnSetFocus(self, msg):
        # Even though we use file change notifications, we should be very sure
        # about it here.
    ...
    def OnUpdateViewFold(self, cmdui):  # Update the tick on the UI.
        if not self.bFolding:
            ...
            return
        id = cmdui.m_nID
        if id in [win32ui.ID_VIEW_FOLD_EXPAND_ALL,
                  win32ui.ID_VIEW_FOLD_COLLAPSE_ALL]:
            cmdui.Enable()
        else:
            enable = 0
    ...
    def FoldTopLevelEvent(self, event=None):
        ...
        try:
            ...
            for lineSeek in range(maxLine):
                if self.SCIGetFoldLevel(
                        lineSeek) & scintillacon.SC_FOLDLEVELHEADERFLAG:
...
class SyntEditTemplate(EditorTemplateBase):
    ...
    def __init__(self, res=win32ui.IDR_TEXTTYPE,
    ...
    def GetPythonPropertyPages(self):
        ...
        return EditorTemplateBase.GetPythonPropertyPages(
            self) + [configui.ScintillaFormatPropertyPage()]
