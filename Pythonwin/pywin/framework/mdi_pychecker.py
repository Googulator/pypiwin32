...
class dirpath:
    ...
    def __add__(self, other):
        if isinstance(other, type(self)) or isinstance(other, type([])):
        ...
        try:
            ...
            if not os.path.isfile(py):
                if "64 bit" in sys.version:
                    py = os.path.join(
                        sys.prefix, 'PCBuild', 'amd64', 'python.exe')
                else:
...
class TheDialog(dialog.Dialog):
    ...
    def __init__(self, dp, fp, gp, cs, r, v):
        ...
        tmp.append([EDIT, gp, 103, (52, 7, 144, 11), CS | win32con.WS_TABSTOP |
    ...
    def OnOK(self):
        self.UpdateData(1)
        for id, name in [(101, 'greppattern'),
                         (102, 'dirpattern'), (103, 'filpattern')]:
            if not self[name]:
