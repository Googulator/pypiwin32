...
def RunScript(defName=None, defArgs=None, bShowDialog=1, debuggingType=None):
    ...
    try:
        ...
        win32ui.MessageBox(
            "The file could not be opened - %s (%d)" %
    ...
    if bWorked:
        win32ui.SetStatusText(
            "Script '%s' returned exit code %s" %
            (script, exitCode))
    else:
        win32ui.SetStatusText(
            'Exception raised while running script  %s' %
            base)
    try:
        sys.stdout.flush()
    except AttributeError:
...
def ImportFile():
    ...
    if pathName is not None:
        if os.path.splitext(pathName)[1].lower(
        ) not in ('.py', '.pyw', '.pyx'):
            pathName = None
...
# Find the Python TabNanny in either the standard library or the Python
