...
#		return self._obj_.SendMessage(msg, w, l)
    ...
    def SCISetTabWidth(self, width):
        # I need to remember the tab-width for the AutoIndent extension.  This
        # may go.
    ...
    def HookHandlers(self):
        ...
        for command, reflection in command_reflectors:
            handler = lambda id, code, ss=self.SendScintilla, tosend=reflection: ss(
                tosend) and 0
...
#		self.HookMessage(self.OnMouseWheel, win32con.WM_MOUSEWHEEL)
    ...
    def OnInitialUpdate(self):
        ...
        # Enable Unicode
        self.SendScintilla(
            scintillacon.SCI_SETCODEPAGE,
            scintillacon.SC_CP_UTF8,
            0)
    ...
    def DoConfigChange(self):
        # Bit of a hack I dont kow what to do about - these should be "editor
        # options"
    ...
    def _AutoComplete(self):
        ...
        if ob is not None:
            try:  # Catch unexpected errors when fetching attribute names from the object
                ...
                try:
                    ...
                    pass
                # The object might be a pure COM dynamic dispatch with typelib
                # support - lets see if we can get its props.
        ...
        if not items:
            ...
            items = [word for word in unique.keys() if word[:2] !=
                     '__' or word[-2:] != '__']
            # Ignore the word currently to the right of the dot - probably a
            # red-herring.
            try:
    ...
    def _UpdateWithITypeInfo(self, items_dict, typeInfo):
        ...
        while len(typeInfos) > 0:
            ...
            if typeAttr.iid not in inspectedIIDs:
                ...
                # E.g. IShellDispatch5 implements IShellDispatch4 which
                # implements IShellDispatch3 ...
    ...
    def OnPrepareDC(self, dc, pInfo):
        # print "OnPrepareDC for page", pInfo.GetCurPage(), "of",
        # pInfo.GetFromPage(), "to", pInfo.GetToPage(), ", starts=",
        # self.starts
        if dc.IsPrinting():
            ...
            if not pInfo.GetPreview() and self.starts is not None:
                ...
                if prevPage > 0 and self.starts[
                        prevPage] >= self.GetTextLength():
                    # All finished.
...
#		print "dev", w, h, l, metrics['tmAscent'], metrics['tmDescent']
        ...
        nextPageStart = self.FormatRange(
            dc, self.starts[pageNum], self.starts[
                pageNum + 1], rc, 1)
