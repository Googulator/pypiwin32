...
class FormatterBase:
    ...
    # Used by the IDLE extensions to quickly determine if a character is a
    # string.
    def GetStringStyle(self, pos):
        try:
            style = self.styles_by_id[self.scintilla.SCIGetStyleAt(pos)]
        except KeyError:
            # A style we dont know about - probably not even a .py file - can't
            # be a string
            return None
    ...
    def LoadPreferences(self):
        self.baseFormatFixed = eval(
            self.LoadPreference(
                "Base Format Fixed", str(
                    self.baseFormatFixed)))
        self.baseFormatProp = eval(
            self.LoadPreference(
                "Base Format Proportional", str(
                    self.baseFormatProp)))
        ...
        for style in list(self.styles.values()):
            ...
            # Use "vanilla" background hardcoded in PYTHON_STYLES if no
...
class Formatter(FormatterBase):
    ...
    def OnStyleNeeded(self, std, extra):
        ...
        # print "enPosPaint %d endStyledChar %d lineEndStyled %d endStyled %d"
        # % (endPosPaint, endStyledChar, lineEndStyled, endStyled)
...
#		trace("After styling, end styled is", self.scintilla.SCIGetEndStyled())
        if self.bCompleteWhileIdle and not self.bHaveIdleHandler and end != - \
                1 and end < scintilla.GetTextLength():
        ...
        while i < lengthDoc:
            ...
            if state == STYLE_DEFAULT:
                ...
                if ch not in wordchars:
                    prevWord = self.ClassifyWord(
                        cdoc, startSeg, i - 1, prevWord)
                    state = STYLE_DEFAULT
                    startSeg = i
                    if ch == '#':
                        if chNext == '#':
