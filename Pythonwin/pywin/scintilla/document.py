...
class CScintillaDocument(ParentScintillaDocument):
    ...
    def _SaveTextToFile(self, view, filename, encoding=None):
        ...
        # encode data before opening file so script is not lost if encoding
        # fails
    ...
    def HookViewNotifications(self, view):
        ...
        parent.HookNotify(
            ViewNotifyDelegate(
                self,
                "OnBraceMatch"),
            scintillacon.SCN_CHECKBRACE)
        parent.HookNotify(
            ViewNotifyDelegate(
                self,
                "OnMarginClick"),
            scintillacon.SCN_MARGINCLICK)
        parent.HookNotify(
            ViewNotifyDelegate(
                self,
                "OnNeedShown"),
            scintillacon.SCN_NEEDSHOWN)

        parent.HookNotify(
            DocumentNotifyDelegate(
                self,
                "OnSavePointReached"),
...
# Delegate to the document, but only from a single view (as each view
# sends it seperately)
