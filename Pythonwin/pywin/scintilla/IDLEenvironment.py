...
class IDLEEditorWindow:
    ...
    #
    def askinteger(self, caption, prompt, parent=None,
                   initialvalue=0, minvalue=None, maxvalue=None):
        while True:
    ...
    def askyesno(self, caption, prompt, parent=None):
        return win32ui.MessageBox(
            prompt, caption, win32con.MB_YESNO) == win32con.IDYES
...
def _NextTok(str, pos):
    ...
    endPos = pos
    while endPos < end and str[
            endPos] not in string.whitespace + string.digits + "+-":
        endPos = endPos + 1
    if pos != endPos:
        return str[pos:endPos], endPos
    return None, 0
...
def TkIndexToOffset(bm, edit, marks):
    ...
    if base.find(".") > 0:
        ...
        try:
            ...
            raise ValueError(
                "Unsupported base offset or undefined mark '%s'" %
                base)
...
# raise IndexError, "Invalid index '%s'" % item
    ...
    def _fix_indexes(self, start, end):
        ...
        while start > 0 and ord(self.edit.SCIGetCharAt(start)) & 0xC0 == 0x80:
            start -= 1
        while end < self.edit.GetTextLength() and ord(
                self.edit.SCIGetCharAt(end)) & 0xC0 == 0x80:
            end += 1
        # now handling fixing \r\n->\n disparities...
        if start > 0 and self.edit.SCIGetCharAt(
                start) == '\n' and self.edit.SCIGetCharAt(start - 1) == '\r':
            start = start - 1
        if end < self.edit.GetTextLength() and self.edit.SCIGetCharAt(
                end - 1) == '\r' and self.edit.SCIGetCharAt(end) == '\n':
            end = end + 1
        return start, end
