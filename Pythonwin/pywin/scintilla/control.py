...
class ScintillaControlInterface:
    ...
    def SCIAddStyledText(self, text, style=None):
        ...
        if style is not None:
            ...
            text = ''.join(text)
        self.SendMessage(
            scintillacon.SCI_ADDSTYLEDTEXT,
            text.encode(default_scintilla_encoding))
    ...
    def SCIBraceHighlight(self, pos, posOpposite):
        return self.SendScintilla(
            scintillacon.SCI_BRACEHIGHLIGHT, pos, posOpposite)
...
#   NOTE - dependent on of we use builtin lexer, so handled below.
    ...
    def SCIStyleSetFont(self, num, name, characterset=0):
        ...
        self.SendScintilla(
            scintillacon.SCI_STYLESETCHARACTERSET,
            num,
            characterset)
    ...
    def SCIMarkerNext(self, lineNo, markerNum):
        return self.SendScintilla(
            scintillacon.SCI_MARKERNEXT, lineNo, markerNum)
...
class CScintillaEditInterface(ScintillaControlInterface):
    ...
    def FindText(self, flags, range, findText):
        ...
        # Scintilla does not handle unicode in EM_FINDTEXT msg (FINDTEXTEX
        # struct)
        ...
        ft_buff = struct.pack(
            ...
            range[0],
            range[1],
            txt_array.buffer_info()[0],
            0,
            0)
        ft_array = array.array('b', ft_buff)
        rc = self.SendScintilla(
            EM_FINDTEXTEX, flags, ft_array.buffer_info()[0])
