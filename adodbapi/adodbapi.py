...
if api.onIronPython:
    ...
    try:
        ...
        import warnings
        warnings.warn(
            "pywin32 package (or IronPython) required for adodbapi.",
            ImportWarning)
    ...
    try:
        if show_value:
            ...
            desc = [
                ...
                (p.Name, adc.directions[p.Direction], adc.adTypeNames.get(p.Type, str(
                    p.Type) + ' (unknown type)'), p.Size, p.Precision, p.NumericScale)
                for p in ADOparameters]
        return '[' + '\n'.join(desc) + ']'
    except:
        return '[]'
...
class Connection(object):
    ...
    def connect(self, kwargs, connection_maker=make_COM_connecter):
        ...
        try:
            ...
            self._raiseConnectionError(
                api.DatabaseError,
                'ADO error trying to Open=%s' %
                self.connection_string)

        if getIndexedValue(self.connector.Properties,
                           'Transaction DDL').Value != 0:
    ...
    def __getattr__(self, item):
        if item == 'rollback':  # the rollback method only appears if the database supports transactions
            ...
            raise AttributeError(
                'no such attribute in ADO connection object as="%s"' %
                item)
...
class Cursor(object):
    ...
    def build_column_info(self, recordset):
        ...
        # if EOF and BOF are true at the same time, there are no records in the
        # recordset
        ...
        for i in range(self.numberOfColumns):
            ...
            try:
                ...
                self._raiseCursorError(
                    api.InternalError,
                    'Data column of Unknown ADO type=%s' %
                    f.Type)
    ...
    def _makeDescriptionFromRS(self):
        ...
        for i in range(self.numberOfColumns):
            ...
            if self.rs.EOF or self.rs.BOF:
                ...
                # TODO: Is this the correct defintion according to the DB API 2
                # Spec ?
            ...
            if parameters_known:  # use ado parameter list
                ...
                if self._parameter_names:  # we expect a dictionary of parameters, this is the list of expected names
                    ...
                    for elem in parameters:
                        ...
                        adotype = api.pyTypeToADOType(elem)
                        p = self.cmd.CreateParameter(
                            name, adotype, adc.adParamInput)  # Name, Type, Direction, Size, Value
                        _configure_parameter(
                            p, elem, adotype, parameters_known)
                        try:
                            self.cmd.Parameters.Append(p)
