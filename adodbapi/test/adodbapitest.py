...
# when Python 2.5 is retired, change that to: from __future__ import
# print_function
"""
...
class CommonDBTests(unittest.TestCase):
        ...
        try:
            ...
            if not self.remote:
                ...
                crsr.execute(
                    ...
                    config.tmp)
                crsr.execute(
                    "INSERT INTO xx_%s(fldData,fld2) VALUES('hey','yo')" %
                    config.tmp)
                # change converter for ALL adoStringTypes columns
                conn.variantConversions[
                    api.adoStringTypes] = duplicatingConverter
                crsr.execute(
                    "SELECT fldData,fld2 FROM xx_%s ORDER BY fldData" %
                    config.tmp)
    ...
    def testUserDefinedConversionForExactNumericTypes(self):
        ...
        #
        if not self.remote and sys.version_info < (
                3, 0):  # Py3 need different test
            oldconverter = adodbapi.variantConversions[
                ado_consts.adNumeric]  # keep old function to restore later
            # By default decimal and "numbers" are returned as decimals.
            # Instead, make numbers return as  floats
            try:
                adodbapi.variantConversions[
                    ado_consts.adNumeric] = adodbapi.cvtFloat
                ...
                # now return strings
                adodbapi.variantConversions[
                    ado_consts.adNumeric] = adodbapi.cvtString
                self.helpTestDataType("numeric(18,2)", 'NUMBER', '3.45')
                # now a completly weird user defined convertion
                adodbapi.variantConversions[
                    ado_consts.adNumeric] = lambda x: u'!!This function returns a funny unicode string %s!!' % x
                self.helpTestDataType("numeric(18,2)", 'NUMBER', '3.45',
                ...
                adodbapi.variantConversions[
                    ado_consts.adNumeric] = oldconverter
    ...
    def helpTestDataType(self, sqlDataTypeString,
        ...
        for inParam in inputs:
            ...
            try:
                crsr.execute(
                    "INSERT INTO xx_%s (fldId,fldData) VALUES (?,?)" %
                ...
                raise
            crsr.execute(
                "SELECT fldData FROM xx_%s WHERE ?=fldID" %
                config.tmp, [fldId])
            rs = crsr.fetchone()
            ...
            if compareAlmostEqual and DBAPIDataTypeString == 'DATETIME':
                iso1 = adodbapi.dateconverter.DateObjectToIsoFormatString(rs[
                                                                          0])
                ...
                assert abs(v - s) / s < 0.00001, \
                    "Values not almost equal recvd=%s, expected=%f" % (rs[
                                                                       0], s)
            else:
                if allowedReturnValues:
                    ok = False
    ...
    def helpCreateAndPopulateTableTemp(self, crsr):
        ...
        for i in range(9):  # note: this poor SQL code, but a valid test
            crsr.execute(
                "INSERT INTO xx_%s (fldData) VALUES (%i)" %
                (config.tmp, i))
            # NOTE: building the test table without using parameter
            # substitution
    ...
    def testExecuteMany(self):
        ...
        crsr.executemany(
            "INSERT INTO xx_%s (fldData) VALUES (?)" %
    ...
    def testRowIterator(self):
        ...
        for inParam in inputs:
            ...
            try:
                ...
                raise
            crsr.execute(
                "SELECT fldTwo,fldThree,fldFour FROM xx_%s WHERE ?=fldID" %
                config.tmp, [fldId])
            rec = crsr.fetchone()
            # check that stepping through an emulated row works
            for j in range(len(inParam)):
                assert rec[j] == inParam[j], 'returned value:"%s" != test value:"%s"' % (rec[
                                                                                         j], inParam[j])
    ...
    def testNamedParamstyle(self):
        ...
        for inParam in inputs:
            ...
            try:
                ...
                raise
            crsr.execute(
                "SELECT fldData FROM xx_%s WHERE :Id=fldID" %
                config.tmp, {
                    'Id': fldId})
            rec = crsr.fetchone()
            self.assertEqual(
    ...
    def testPyformatParamstyle(self):
        ...
        for inParam in inputs:
            ...
            try:
                ...
                raise
            crsr.execute(
                "SELECT fldData FROM xx_%s WHERE %%(Id)s=fldID" %
                config.tmp, {
                    'Id': fldId})
            rec = crsr.fetchone()
            self.assertEqual(
    ...
    def testAutoRollback(self):
        ...
        assert row is None, 'cursor.fetchone should return None if a query retrieves no rows. Got %s' % repr(
            row)
        self.helpRollbackTblTemp()
...
class TestADOwithSQLServer(CommonDBTests):
    ...
    def setUp(self):
        self.conn = config.dbSqlServerconnect(
            *config.connStrSQLServer[0],
            **config.connStrSQLServer[1])
    ...
    def testVariableReturningStoredProcedure(self):
        ...
        assert retvalues[0] == 'Dodsworth', '%s is not "Dodsworth"' % repr(retvalues[
                                                                           0])
        assert retvalues[1] == 'Anne', '%s is not "Anne"' % repr(retvalues[1])
        assert retvalues[2] == 'DodsworthAnne', '%s is not "DodsworthAnne"' % repr(retvalues[
                                                                                   2])
        self.conn.rollback()
    ...
    def testDatetimeProcedureParameter(self):
        ...
        result = crsr.callproc(
            'sp_DeleteMeOnlyForTesting', [
                adodbapi.Timestamp(
                    2014, 12, 25, 0, 1, 0), 'Beep', ' ' * 30])

        assert result[
            2] == 'Dec 25 2014 12:01AM Beep', 'value was="%s"' % result[2]
        self.conn.rollback()
    ...
    def testIncorrectStoredProcedureParameter(self):
        ...
        # calling the sproc with a string for the first parameter where a
...
class TestADOwithPostgres(CommonDBTests):
    ...
    def setUp(self):
        self.conn = config.dbPostgresConnect(
            *config.connStrPostgres[0],
            **config.connStrPostgres[1])
        self.conn.timeout = 30  # turn timeout back up
...
class TestPythonTimeConverter(TimeConverterInterfaceTest):
    ...
    def testCOMDate(self):
        mk = time.mktime(
            (2002, 6, 28, 18, 15, 1, 4, 31 + 28 + 31 + 30 + 31 + 28, -1))
        t = time.localtime(mk)
    ...
    def testDate(self):
        t1 = time.mktime(
            (2002, 6, 28, 18, 15, 1, 4, 31 + 28 + 31 + 30 + 31 + 30, 0))
        t2 = time.mktime(
            (2002, 6, 30, 18, 15, 1, 4, 31 + 28 + 31 + 30 + 31 + 28, 0))
